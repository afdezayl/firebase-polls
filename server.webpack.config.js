...
const webpackDetectedDependencies = [];
/**
...
module.exports = (config, context) => {
  ...
  config.plugins.push(new AccessDependenciesPlugin());
        ...
        compilation.hooks.finishModules.tap(
          ...
          (modules) => {
            ...
            console.log(
              modules
                .filter((m) => m.external)
                .map((m) => m.issuer.dependencies.map((d) => d.name))
            );
            webpackDetectedDependencies.push(deps);
          }
        );
      }
    );
...
function generatePackageJson() {
  const implicitDeps = ['reflect-metadata', 'class-transformer', 'rxjs'];

  const dependencies = [...implicitDeps, ...webpackDetectedDependencies].reduce(
    (acc, dep) => {
  ...
  console.log('Implicit dependencies:', basePackageJson);
